...
class Like extends \App\Base
{
    ...
    {
        $res = $this->db->read("SELECT {$this->table_name}.* from {$this->table_name}  where {$this->entity_id_name} = {$this->entity_id}");
        if (!$res)
            $this->data = [];
        else
            $this->data = $res;
    ...
    public function doLike($user_id)
    {
        ...
        $res = $this->db->write("INSERT into {$this->table_name} (`{$this->entity_id_name}`,`user_id`) VALUES ($this->entity_id,?)",[$user_id]);
        $this->loadLikes();
        if ($res)
    ...
    public function removeLike($user_id)
    {
        ...
        $res = $this->db->write("DELETE from {$this->table_name} where {$this->entity_id_name} = {$this->entity_id} and user_id = ?",[$user_id]);
       $this->loadLikes();
        if ($res)
            return true;
        return false;
